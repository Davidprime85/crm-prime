          ...
          <div className="flex flex-col items-start gap-2">
             <div className="w-full flex justify-center md:justify-start">
                <img 
                  ...
                  className="h-20 w-auto object-contain bg-white rounded px-2 py-1"
                  onError={(e) => {
                    e.currentTarget.style.display = 'none';
                    // Se a imagem falhar, mostra este bloco
                    const fallback = document.getElementById('logo-fallback-sidebar');
                    if(fallback) fallback.style.display = 'block';
                  }}
                />
                <div id="logo-fallback-sidebar" style={{display: 'none'}}>
                    <h1 className="text-2xl font-bold text-amber-500 tracking-tighter leading-none">GRUPO<br/><span className="text-white">PRIME</span></h1>
                </div>
             </div>
          </div>
        </div>
