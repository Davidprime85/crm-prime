...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ initialTab = 'dashboard' }) => {
  ...
  const handleCreateClient = async (e: React.FormEvent) => {
    ...
    if (result.success) {
        alert(`Cliente ${newClient.name} pré-cadastrado com sucesso!\n\nIMPORTANTE: Agora vá ao painel do Supabase (Authentication) e convide/crie o usuário com o e-mail: ${newClient.email}`);
  ...
  const renderSettings = () => (
      <div className="max-w-2xl mx-auto">
          <div className="bg-white p-8 rounded-xl shadow-sm border border-slate-100 mb-8">
              ...
              <p className="text-sm text-slate-500 mb-4">Para adicionar atendentes, crie um usuário no painel do Supabase (Authentication) e adicione na tabela 'profiles' com a role 'attendant'.</p>
              
              <div className="bg-slate-100 p-4 rounded text-xs text-slate-600 font-mono">
                 Devido à segurança, o cadastro de novos usuários administrativos deve ser feito diretamente no painel do banco de dados ou via convite por e-mail (funcionalidade futura).
              </div>
          </div>
      </div>
  );
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 print:hidden">
        <div>
          ...
          <p className="text-slate-500">Controle total da operação.</p>
        </div>
